...
const App = () => {
  ...
  React.useEffect(() => {
    ...
    const bootstrapAsync = async () => {
      ...
      dispatch({ type: 'RESTORE_TOKEN', userId: userId,  generatedToken: userGeneratedToken});
    };
    bootstrapAsync();
  }, []);
  ...
  const authContext = React.useMemo(
    () => ({
          ...
          .then(async (data) => {
            ...
            if(data !== "user does not exist"){
              if (data === "incorrect password") {
                ...
                dispatch({ type: 'SIGN_IN', generatedToken: data.token, userId: data.user._id, 
                  userName: data.user.name, userLastName: data.user.lastname, userMail: data.user.email,
                  userBirthdate: data.user.birthdate, userPhoto: data.user.photo, 
                  password1: data.user.password.substring(0, 7), 
                  password2: data.user.password.substring(7, data.user.password.length), 
                });
                setLoggedIn(true);
              }
            } else {
      ...
      signUp: async (data) => {
        //To fulfill with sign up logic to change auth context
      },
